...
public class ExpenseService {
    ...
    @Transactional(readOnly = true)
    public List<ExpenseDTO> listExpensesLastWeek(String userEmail) {
        User user = userRepository.findByEmailIgnoreCase(userEmail)
                .orElseThrow(() -> new IllegalStateException("User not found."));

        LocalDate today = LocalDate.now();
        // Last week = last 7 days (including today, so 6 days ago to today)
        LocalDate lastWeekStart = today.minusDays(6);
        LocalDate lastWeekEnd = today;

        return expenseRepository.findAllByUserAndTransactionDateBetween(user, lastWeekStart, lastWeekEnd)
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
    ...
    @Transactional(readOnly = true)
    public List<ExpenseDTO> listExpensesLastMonth(String userEmail) {
        User user = userRepository.findByEmailIgnoreCase(userEmail)
                .orElseThrow(() -> new IllegalStateException("User not found."));

        LocalDate today = LocalDate.now();
        // Last month = last 30 days (including today, so 29 days ago to today)
        LocalDate lastMonthStart = today.minusDays(29);
        LocalDate lastMonthEnd = today;

        return expenseRepository.findAllByUserAndTransactionDateBetween(user, lastMonthStart, lastMonthEnd)
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
    ...
    @Transactional(readOnly = true)
    public List<ExpenseDTO> listExpensesLastYear(String userEmail) {
        User user = userRepository.findByEmailIgnoreCase(userEmail)
                .orElseThrow(() -> new IllegalStateException("User not found."));

        LocalDate today = LocalDate.now();
        // Last year = last 365 days (including today, so 364 days ago to today)
        LocalDate lastYearStart = today.minusDays(364);
        LocalDate lastYearEnd = today;

        return expenseRepository.findAllByUserAndTransactionDateBetween(user, lastYearStart, lastYearEnd)
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
    ...
    @Transactional(readOnly = true)
    public List<ExpenseDTO> listExpensesByDateRange(String userEmail, LocalDate startDate, LocalDate endDate) {
        User user = userRepository.findByEmailIgnoreCase(userEmail)
                .orElseThrow(() -> new IllegalStateException("User not found."));
        ...
        return expenseRepository.findAllByUserAndTransactionDateBetween(user, startDate, endDate)
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
